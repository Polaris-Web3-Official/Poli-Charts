...
//------------------------------------------------------------------------------------------------------------------------
export async function OwnersBarChart(options) {
  const {
  ...
  async function renderChart() {
    ...
    // Proxy para observar cambios en fetchData después de renderizar el gráfico
    const handler = {
      set(target, prop, value) {
        if (prop === "amount") {
          target[prop] = value;
          updateChart(chart); // Actualiza el gráfico cuando amount cambie
          return true;
        }
        target[prop] = value;
        return true;
    ...
    // Asegúrate de usar proxyFetchData en lugar de fetchData directamente
